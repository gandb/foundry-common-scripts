export class CacheReturnControl<K,V>{
    ...
    private _cache:Map<number,V> = new Map();
    private _indexKey:Map<K,number> = new Map(); 
    private _firstIndex:number = 0;
    private _nextIndex:number = 0;  

    public constructor(private capacity:number = 1000){
    ...
    private cleanCacheLazy() {
        ...
        for (let index: number = 0; index < keysToRemove.length; index++) {
            const key: K = keysToRemove[index];
