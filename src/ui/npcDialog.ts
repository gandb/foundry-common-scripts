...
Hooks.on("onReadyDialogUtils", async (data:any) => {
	...
	commonModule.debug("onReadyDialogUtils ...",commonModule) ;  
	...
	Hooks.on("getSceneControlButtons", async (controls:any) => { 
				...
				commonModule.debug("On getSceneControlButtons 05...",controls,npcDialog,docNpcDialog.COMMON_MODULE.DIALOG_UTILS,commonModule) ;


		 
		commonModule.debug("On getSceneControlButtons...") ;  
	 
			commonModule.debug("On getSceneControlButtons...:10 controls:",controls) ;  
		...
		commonModule.debug("On getSceneControlButtons...:20") ;   
		Hooks.callAll("onReadyNPCDialog",{});
		commonModule.debug("On getSceneControlButtons 40...",controls,npcDialog,docNpcDialog.COMMON_MODULE.DIALOG_UTILS,commonModule) ;
...
export class NPCDialog {
	...
	constructor(){
		...
		commonModule =docNpcDialog.COMMON_MODULE;
		commonModule.debug("NPCDialog constructor");
		dialogUtils = commonModule.DIALOG_UTILS;  
	...
	public async addNPCButtons (controls:any) {
		npcDialog =  dialogUtils.NPC_DIALOG ;
			...
			commonModule.debug("Criando botão dos NPCs especiais",controls);
	...
	public async showNPCChooseDialog () {
		...
		commonModule.debug("showNPCChooseDialog:10 before creating buttons",npcDialog);		

		let buttons = [];
		
		npcDialog.npcs.forEach(npc=>
		{
			commonModule.debug("showNPCChooseDialog:15 add NPC " , npc.name , npc);	
   ...
   public async callNPC (npc:NPC)
	{
		commonModule.debug("Selecionado ...", npc);
