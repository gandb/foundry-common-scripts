...
export abstract class NPC {
	...
	public async init() {
		...
		await this.whaitFor(() => injectController.resolve("NPCDialog"), fiveMinutes);
		if (!injectController.resolve("NPCDialog")) {
			throw new Error("Timeout waiting for hooks");
		}
		await this.whaitFor(() => injectController.resolve("DialogUtils"), fiveMinutes);
		...
		if (!submits) {
			...
			submits = [
				dialogUtils.createButton("send", "Enviar", true, "action", async () => {
					...
					const queryResult = document.querySelector(`.${alias}-actions-buttons SELECT`) as HTMLSelectElement || null;
					...
					loguer.debug("NPC.createDialog [20]: depois de selecionar o resultado", result);
				...
				dialogUtils.createButton("cancel", "Cancelar", true, "action", async () => {
