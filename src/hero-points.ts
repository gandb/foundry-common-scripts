 ...
 * funcoes de log do commons por isto tem que aguardar
...
function initializeHabilityHero(){
	...
	//data-ability="hon"
		...
		if(htmldivs.length==0)
		{
			doc.COMMON_MODULE.warning(`Hability hero not enable to convert for hero, your cmpaing not use? Code error: 01`);
			return;
		}
		...
		{
			doc.COMMON_MODULE.warning(`Hability hero not enable to convert for hero, your cmpaing not use? Code error: 02`);
			return;
		}
		...
		if(!game.user.isGM){
			...
			{
				doc.COMMON_MODULE.warning(`Hability hero not enable to convert for hero, your cmpaing not use? Code error:03`);
				return;
			}
